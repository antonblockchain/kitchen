<template>
  <div class="calc__file">
    <button
      v-show="picture.length > 0"
      type="button"
      class="calc__file_delete calc__nav_link"
      @click="deletePhoto"
    >
    ...
    <label class="order__file order__label">
      ...
      <input
        ...
        @change="previewImage"
      />
      <span class="order__file_content" v-show="picture.length === 0">
        <svg
          width="32"
          height="40"
      ...
      <span
        class="order__file_content"
        v-show="picture.length > 0"
        v-for="img in picture"
        :key="img"
      >
        <img :src="img" style="width: 4rem" alt="img" />
      </span>
    </label>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    // currentItem() {
    //   return this.currentOrder.find(item => item.name === this.item.name);
    // },
    picture: {
      get() {
  ...
  methods: {
    previewImage(event) {
      ...
      if (this.imageData) {
        ...
        for (let i = 0; i < Math.min(this.imageData.length, 3); i++) {
          this.onUpload(this.imageData[i], i);
        }
      }
    },
    deletePhoto() {
    ...
    onUpload(imageData, id) {
      this.showProgress = true;
      const storageRef = firebase
        .storage()
        .ref(
          `${this.currentOrder.order}_${this.item.name}_${id}_${this.currentOrder.user}`
        )
        .put(imageData);
      storageRef.on(
        `state_changed`,
